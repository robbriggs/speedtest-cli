...
def speedtest():
    ...
    parser.add_argument('--rawsize', action='store_true', 
                        help='Show speed in raw values')
    ...
    if not args.simple:
        print_()
    if args.rawsize:
        print_('Download: %0.2f' % (dlspeed * args.units[1]))
    else:
    ...
    if not args.simple:
        print_()
    if args.rawsize:
        print_('Upload: %0.2f' % (ulspeed * args.units[1]))
    else:
